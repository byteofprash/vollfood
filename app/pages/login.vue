...
const stage = ref<'form' | 'check-email'>('form')
...
async function submit() {
  ...
  try {
    if (mode.value === 'login') {
      ...
      const redirect = new URL(`${window.location.origin}/confirm`)
      if (inviteCode.value) redirect.searchParams.set('invite', inviteCode.value)
...
async function resendEmail() {
  ...
  try {
    ...
    error.value = e.message ?? 'Failed to resend.'
  } finally {
...
<template>
  <div class="min-h-screen bg-vf-bg flex flex-col overflow-y-auto pb-10 lg:justify-center lg:items-center">
    <div class="w-full lg:max-w-sm lg:w-full">
      ...
      <!-- ── Check email state ── -->
      <template v-if="stage === 'check-email'">
        <div class="flex flex-col items-center px-7 pt-20 pb-8 text-center gap-4">
          <div class="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
              <rect x="2" y="4" width="20" height="16" rx="3" stroke="#C8622A" stroke-width="1.8"/>
              <path d="M2 7l10 7 10-7" stroke="#C8622A" stroke-width="1.8" stroke-linecap="round"/>
            </svg>
          </div>
          <div>
            <p class="text-[20px] font-bold text-vf-text">Check your inbox</p>
            <p class="text-sm text-vf-muted mt-1.5 leading-relaxed">
              We sent a confirmation link to<br>
              <span class="font-semibold text-vf-text">{{ email }}</span>
            </p>
            <p class="text-xs text-vf-muted mt-2">Click the link in the email to complete your account.</p>
          </div>
        </div>
        ...
        <div class="mx-5 flex flex-col gap-3">
          <button
            :disabled="resending"
            class="py-3.5 bg-vf-card border border-vf-border rounded-2xl text-[15px] font-semibold text-vf-text disabled:opacity-60 transition-opacity"
            @click="resendEmail"
          >
            {{ resending ? 'Sending…' : resendSent ? 'Email sent ✓' : 'Resend confirmation email' }}
          </button>
          <button
            class="py-3 text-sm text-vf-muted"
            @click="stage = 'form'; mode = 'login'; error = ''"
          >
            Back to login
          </button>
        </div>

        <p v-if="error" class="text-red-500 text-[13px] text-center mx-6 mt-3">{{ error }}</p>
      </template>
      ...
      <template v-else>
        ...
        <div class="mx-5 bg-vf-card rounded-2xl border border-vf-border overflow-hidden">
          <!-- Name (signup only) -->
          <div v-if="mode === 'signup'" class="px-4 py-3.5 border-b border-vf-border">
            <div class="text-[11px] font-semibold text-vf-muted uppercase tracking-wide mb-1">Your Name</div>
            <input
              v-model="name"
              type="text"
