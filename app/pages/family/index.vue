    ...
    <div class="flex-1 overflow-y-auto bg-vf-bg pb-20">
      ...
      <div class="border-t border-b border-vf-border">
        <NuxtLink
          ...
          :key="m.id"
          :to="`/family/${m.id}`"
          :class="i < (members?.length ?? 0) - 1 ? 'border-b border-vf-border' : ''"
          class="flex items-center gap-3 px-4 py-3.5 bg-vf-card no-underline transition-colors hover:bg-vf-surface"
        >
          <VfAvatar
            :name="m.name ?? ''"
